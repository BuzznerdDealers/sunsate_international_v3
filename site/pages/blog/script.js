...
(function () {
  ...
  var plural = function (shown) {
    if (count) count.textContent = shown + (shown === 1 ? ' article' : ' articles');
  };

  // Before any chip is pressed, too: the last page of the blog can hold a
  // single post, and "1 articles" is what the count template alone would say.
  plural(section.querySelectorAll('[data-bz-part~="item"]').length);
